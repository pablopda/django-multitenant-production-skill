...
- From the post-rewrite adversarial review (40 surviving findings, the confirmed ones fixed): the edit-time hook no longer absorbs unreported findings from other files into its baseline (they now report when their own file is edited); a bare "celery" substring (import/comment) no longer flips request-path files into strict/task mode — only real task decorators do; the schema-mode signals exemption covers `signals/` packages, not just `*signals.py`; `resolve_schema_mode` weighs declared dependencies above prose mentions, so a README comparing django-multitenant cannot demote a django-tenants project; `MIDDLEWARE` order handles `+` concatenation and `[*BASE, ...]` without miscounting; multi-line queryset assignments dedupe; compound filenames (`restapi.py`, `myviews.py`) keep their v1.1.0 high-risk coverage; `tenant_a`/`tenant_b` fixtures are recognized even under `--tenant-term`; scaffold-snippet files can't satisfy the settings-union checks; `CACHE-GLOBAL-KEY` downgrades to Low when a tenant-aware `KEY_FUNCTION` is configured; hooks prefer `CLAUDE_PROJECT_DIR` over the drifting payload cwd, use a per-user 0700 fallback data dir, and write baselines atomically; the scaffold errors cleanly when a dotted-app component collides with a file; the generator rejects directory `--output`s.
